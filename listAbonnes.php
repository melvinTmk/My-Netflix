...
if(!empty($_GET['action']))
{
    ...
    $query = 'SELECT DISTINCT m.pseudo, r.id_demandeur, m.id FROM membres m, relation r WHERE  r.id_receveur = ?  AND r.statut = 2 AND m.id = r.id_demandeur';
    $req = $bdd->prepare($query);
    $req->execute(array($id_membre_connecté));
if($req->rowCount() > 0)
{
    echo ' <h1 style="text-align:center; font-size:3em; color:white;">Vous avez   
   '.$req->rowCount().' abonné(s)
...
{
    echo "<h3 style='text-align:center; color:white;font-size:3.5em;'>Pas d'abonné ...</h3>";
}
